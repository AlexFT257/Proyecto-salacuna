...
export const ModalUpdateParvulos = ({ actividad, setActividadData, setShowModalUpdateParvulos }) => {
        ...
        try {
            ...
            if (changeParvulos) {
                ...
                if (res.status === 200) {
                    ...
                    const parvulosObjects = parvulos.filter((parvulo) => putParvulos.includes(parvulo._id));
    ...
    return (
      ...
      <div className="fixed top-0 left-0 w-screen h-screen bg-black bg-opacity-50 z-50">
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-1/2 h-fit max-md:w-11/12 bg-white rounded-2xl border-2 border-slate-900 shadow-lg shadow-slate-900">
            <div className="flex flex-col justify-center items-center m-5 space-y-3">
              <h1 className="text-2xl font-bold">Modificar parvulos de la Actividad</h1>
